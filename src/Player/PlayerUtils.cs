...
namespace SharpTimer
{
    public partial class SharpTimer
    {
        ...
        public string GetPlayerPlacement(CCSPlayerController? player)
        {
            ...
            foreach (var kvp in SortedCachedRecords!.Take(999))
            {
        ...
        public async Task PrintMapTimeToChat(CCSPlayerController player, string steamID, string playerName, int oldticks, int newticks, int bonusX = 0, int timesFinished = 0, int style = 0, int prevSR = 0)
        {
            ...
            Server.NextFrame(() =>
            {
                ...
                {
                    if (bonusX != 0) player.PrintToChat(Localizer["map_finish_bonus", playerName, bonusX]);
                    else             player.PrintToChat(Localizer["map_finish", playerName]);
        ...
        public async Task PrintStageTimeToChat(CCSPlayerController player, string steamID, string playerName, int oldticks, int newticks, int stage, int bonusX = 0, int prevSR = 0)
        {
            ...
            Server.NextFrame(() =>
            {
                if (newSR)
                {
                    ...
                    // Print cp/stage record localizer depending on map type
                    var recordKey = (useStageTriggers || !useCheckpointTriggers)
                        ? "new_stage_server_record"
                        : "new_cp_server_record";
                    PrintToChatAll(Localizer[recordKey, playerName]);
