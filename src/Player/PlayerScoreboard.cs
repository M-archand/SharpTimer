...
public partial class SharpTimer
{
    ...
    {
        foreach (var player in connectedPlayers.Values.Where(player => player.IsValid))
        {
    ...
    private void assignScoreboard(CCSPlayerController player)
    {
        ...
        if (timer.IsAddingStartZone || timer.IsAddingEndZone || timer.IsAddingBonusStartZone || timer.IsAddingBonusEndZone)
        {
            return;
        }
        ...
        {
            matchStats.Kills = timer.IsBonusTimerRunning ? -timer.BonusStage : timer.CurrentMapStage;
        }
