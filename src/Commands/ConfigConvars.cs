...
namespace SharpTimer
{
    ...
    {
        private bool _hasWarnedAboutMissingFakeConvars = false;
        ...
        private void CheckMissingFakeConvars()
        {
            try
            {
                ...
                {
                    SharpTimerDebug($"[CFG] SharpTimer/config.cfg not found at '{cfgPath}'. Skipping fake convar check.");
                    return;
                }

                // Parse which convar names are in config.cfg
                ...
                // Log missing fake convars
                var missing = expected.Except(present).OrderBy(x => x).ToList();

                if (_hasWarnedAboutMissingFakeConvars)
                    return;

                SharpTimerDebug($"[CFG] {missing.Count} fake convar(s) are not set in cfg/SharpTimer/config.cfg:");
                foreach (var name in missing)
                    SharpTimerDebug($"[CFG]   - {name} (Falling back to default)");

                _hasWarnedAboutMissingFakeConvars = true;
            }
            catch (Exception ex)
            {
                SharpTimerError($"Error in CheckMissingFakeConvars: {ex.Message}");
            }
        }
        ...
        public void SharpTimerJumpStatsUnlockerCapValueConvar(CCSPlayerController? player, CommandInfo command)
        {
            ...
            if (float.TryParse(args, out float value) && value > 0)
            {
                ...
                if (movementUnlockerCapEnabled) 
                    SharpTimerDebug($"SharpTimer JumpStats Movement Unlocker cap value set to {value} units.");
            }
            else
            {
        ...
        public void SharpTimerGlobalCacheConvar(CCSPlayerController? player, CommandInfo command)
        {
            ...
            {
                SharpTimerConPrint("Invalid global cache refresh interval value. Please provide a positive float.");
            }
        }
        ...
        public void SharpTimerReplaysUseDatabase(CCSPlayerController? player, CommandInfo command)
        {
            ...
            useDatabaseReplays = bool.TryParse(args, out bool useDatabaseReplaysValue) ? useDatabaseReplaysValue : args != "0" && useDatabaseReplays;
        }
    }
}